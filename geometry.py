...
def coordinates_geometry(V, H, h_leg, v_leg, gv, gh, h_mid, cv, ch, mh, mv, gap, gap_m):
    """
    Basic figures
    """
...
def femm_geometry(data):
    """
        Create FEMM geometry + materials using dimentions from `coordinates_geometry()`
    """

    def _base_geometry(cbg, data, mat_coil_name):
        ...
        # mirror CORE and COILS

        x_m = (data["H"]/2) + data["ch"] + (1.5 * data["gh"])
        y_m_max = (data["mv"] / 2) + data["gap"] + data["V"] + data["gv"]
        y_m_min = (data["mv"] / 2) + (data["gap"] / 2)

        # top - to right
        femm.mi_selectrectangle(-x_m, y_m_max, x_m, y_m_min, 4)
        femm.mi_mirror2(x_m, - y_m_max, x_m, y_m_max,1)
        femm.mi_clearselected()
        femm.mi_selectrectangle(-x_m, y_m_max, x_m, y_m_min, 4)
        femm.mi_mirror2(x_m, - y_m_max, x_m, y_m_max,2)
        femm.mi_clearselected()
        ...
        dx_magnet = data["mh"] + data["gap_m"]
        copies_magnet=3

        # left - to left
        femm.mi_selectrectangle(-x_m, y_m_min, 0, -y_m_min, 4)
        femm.mi_copytranslate2(-dx_magnet, 0, copies_magnet, 1)
        femm.mi_clearselected()
        femm.mi_selectrectangle(-x_m, y_m_min, 0, -y_m_min, 4)
        femm.mi_copytranslate2(-dx_magnet, 0, copies_magnet, 2)
        femm.mi_clearselected()

        femm.mi_selectlabel(cbg["magn_left_label"][0]-dx_magnet, cbg["magn_left_label"][1])
        femm.mi_setblockprop(data["magnet_material"], 0, 1, '<None>', 180, 0, 0)
        femm.mi_clearselected()

        femm.mi_selectlabel(cbg["magn_left_label"][0]-(dx_magnet*copies_magnet), cbg["magn_left_label"][1])
        femm.mi_setblockprop(data["magnet_material"], 0, 1, '<None>', 180, 0, 0)
        femm.mi_clearselected()
        ...
        femm.mi_selectlabel(cbg["magn_right_label"][0]+dx_magnet, cbg["magn_right_label"][1])
        femm.mi_setblockprop(data["magnet_material"], 0, 1, '<None>', 0, 0, 0)
        femm.mi_clearselected()

        femm.mi_selectlabel(cbg["magn_right_label"][0]+(dx_magnet*copies_magnet), cbg["magn_right_label"][1])
        femm.mi_setblockprop(data["magnet_material"], 0, 1, '<None>', 0, 0, 0)
        femm.mi_clearselected()
    ...
    cbg = coordinates_geometry(data["V"], data["H"], data["h_leg"], data["v_leg"], data["gv"], data["gh"], 
        data["h_mid"], data["cv"], data["ch"], data["mh"], data["mv"], data["gap"], data["gap_m"])

    make_femm_geometry(cbg)

    dx_magnet = _base_geometry(cbg, data, mat_coil_name)
    ...
    # input for `calculate()`
    return dx_magnet, cbg, data


def calculate(dx_magnet, cbg, data):
    """
    Calculate `femm_geometry()` and analize
    ...
    femm.mo_selectblock(cbg["magn_left_label"][0]-dx_magnet, cbg["magn_left_label"][1])
    femm.mo_selectblock(cbg["magn_left_label"][0]-(dx_magnet*2), cbg["magn_left_label"][1])
    femm.mo_selectblock(cbg["magn_left_label"][0]-(dx_magnet*3), cbg["magn_left_label"][1])

    femm.mo_selectblock(*cbg["magn_right_label"])
    femm.mo_selectblock(cbg["magn_right_label"][0]+dx_magnet, cbg["magn_right_label"][1])
    femm.mo_selectblock(cbg["magn_right_label"][0]+(dx_magnet*2), cbg["magn_right_label"][1])
    femm.mo_selectblock(cbg["magn_right_label"][0]+(dx_magnet*3), cbg["magn_right_label"][1])
