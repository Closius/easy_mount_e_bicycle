...
def c_block_geometry(V, H, h1, h2, h3, v1, v2, v3, k, m, gap_V, shift_center_center):
    ...
    core_bttm = [(x[0], -x[1] - v_mirror) for x in core_top]

    # move and locate: coils
    gv_bttm = v_leg - cv - gv
    h_mid_coil_gap = h_mid - (2*ch) + (2*gh)
    coil_top_1 = [(x[0] - h_move - ch - gh, x[1] + gap + (mv/2) + gv_bttm) for x in coil]
    coil_top_2 = [(x[0] + (2*ch) + (2*gh) + h_leg, x[1]) for x in coil_top_1]
    coil_top_3 = [(x[0] + ch + h_mid_coil_gap, x[1]) for x in coil_top_2]
    coil_top_4 = [(x[0] + (2*ch) + (2*gh) + h_leg, x[1]) for x in coil_top_3]

    coil_bttm_1 = [(x[0], -x[1] - v_mirror) for x in coil_top_1]
    coil_bttm_2 = [(x[0], -x[1] - v_mirror) for x in coil_top_2]
    coil_bttm_3 = [(x[0], -x[1] - v_mirror) for x in coil_top_3]
    coil_bttm_4 = [(x[0], -x[1] - v_mirror) for x in coil_top_4]

    return e_top, e_bttm, magn
